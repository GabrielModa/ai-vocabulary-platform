# Documentation index

Documents describe the approved product and engineering baseline. When documents conflict, an
accepted entry in `DECISIONS.md` wins, followed by the more specific document.

## Product and experience

- [Product vision](PRODUCT_VISION.md)
- [Product rules](PRODUCT_RULES.md)
- [Project principles](PROJECT_PRINCIPLES.md)
- [Design system](DESIGN_SYSTEM.md)
- [UX guidelines](UX_GUIDELINES.md)
- [Roadmap](ROADMAP.md)

## Learning and motivation

- [Learning engine](LEARNING_ENGINE.md)
- [Spaced repetition](SPACED_REPETITION.md)
- [Game engine](GAME_ENGINE.md)

## Architecture and data

- [Architecture](ARCHITECTURE.md)
- [Database](DATABASE.md)
- [API](API.md)
- [AI system](AI_SYSTEM.md)
- [Prompt library](PROMPT_LIBRARY.md)
- [Tech stack](TECH_STACK.md)
- [Scalability](SCALABILITY.md)
- [Performance](PERFORMANCE.md)

## Delivery and assurance

- [Security](SECURITY.md)
- [Testing](TESTING.md)
- [CI/CD](CI_CD.md)
- [Deployment](DEPLOYMENT.md)
- [Decisions](DECISIONS.md)
- [Risks](RISKS.md)

The architecture documents currently define constraints and ownership. Detailed component, contract,
schema, and deployment designs are completed in the architecture milestone.
